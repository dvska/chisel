...
def _visualize(target):
    ...
    if fb.evaluateBooleanExpression(
                            '(unsigned long)CFGetTypeID((CFTypeRef)' + target + ') == (unsigned long)CGImageGetTypeID()'):
        ...
        if objectHelpers.isKindOfClass(target, 'UIImage'):
            ...
            print '{} is not supported. You can visualize UIImage, CGImageRef, UIView, or CALayer.'.format(
