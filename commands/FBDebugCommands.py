...
class FBWatchInstanceVariableCommand(fb.FBCommand):
    ...
    def run(self, arguments, options):
        ...
        error = lldb.SBError()
        watchpoint = lldb.debugger.GetSelectedTarget().WatchAddress(objectAddress + ivarOffset, ivarSize, False, True,
                                                                    error)
...
class FBMethodBreakpointCommand(fb.FBCommand):
    ...
    def run(self, arguments, options):
        ...
        if not found:
            print 'There doesn\'t seem to be an implementation of {} in the class hierarchy. Made a boo boo with the selector name?'.format(
                selector)
            return
